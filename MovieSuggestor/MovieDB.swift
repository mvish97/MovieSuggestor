...
class MovieDB {
    ...
    func getShowList(type: String) {
        
        let url = BASE_URL + "/tv/\(type)?" + API_KEY + LANG + "&page=1"
        
        var tvList = [MovieModel]()
            ...
            if let result = response.result.value as? Dictionary<String,Any> {
                if let list = result["results"] as? [Dictionary<String,Any>] {
                    ...
                    for i in list {
                        ...
                        tvList.append(MovieModel(poster: #imageLiteral(resourceName: "tempPoster"),
                                                 background: UIImage(),
